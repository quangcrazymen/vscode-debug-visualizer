...
import { enableHotReload, hotRequireExportedFn, registerUpdateReconciler, getReloadCount } from "@hediet/node-reload";
...
export function deactivate() {
	window.showInformationMessage("Shutdown the extension");
}
...
export class Extension {
	...
	private readonly debuggerView = this.dispose.track(new DebuggerViewProxy(this.debugger));
	...
	public readonly dataSource = new VisualizationWatchModelImpl(
		new DispatchingVisualizationBackend(
			new ComposedVisualizationSupport([
				new ConfigurableVisualizationSupport(this.config, this.debuggerView),
	...
	private readonly views = this.dispose.track(new InternalWebviewManager(this.server, this.config));
	...
	constructor() {
		...
		this.dispose.track(
			commands.registerCommand("vscode-debug-visualizer.new-visualizer", () => {
				this.views.createNew();
			})
		);

		this.dispose.track(
			commands.registerCommand("vscode-debug-visualizer.visualizer-set-expression", () => {
				...
				if (selection.isEmpty) {
					const lineText = editor.document.lineAt(selection.start).text;
					const regexp = /`(.*)`/g;
					selectedText = "";
					let match;
					while ((match = regexp.exec(lineText))) {
						if (
							...
							selection.start.character <= match.index + match[0].length
						) {
